...
import type {MenuItem, Staff, Settings} from '../types'
...
export default function OrderBuilder({
  ...
  const loggedUser=(()=>{
    try{
      return JSON.parse(
        localStorage.getItem('mahi_user')||'null'
      )
    }catch{
      return null
    }
  ...
  const load=async()=>{
    ...
    const [m,s,c,st,sh,h]=await Promise.all([
      api<MenuItem[]>('/menu'),
      api<Staff[]>('/staff'),
  ...
  /*
    ...
    Example:
    Menu price = AED 12
    Customer pays = AED 12
    VAT is automatically extracted
    from inside AED 12.
  */
  ...
  /*
    OPEN ONE SHIFT FOR THE
    CASH DRAWER / TERMINAL.

    Staff sales themselves are
    still saved against logged-in
    staff.
  */
