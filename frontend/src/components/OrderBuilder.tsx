...
export default function OrderBuilder({waiterMode=false,cashierCompact=false}:{waiterMode?:boolean;cashierCompact?:boolean}){
  ...
  const categories=useMemo(()=>Array.from(new Set(menu.map(x=>x.category).filter(Boolean))),[menu])
  useEffect(()=>{
    if(categories.length && (!category || !categories.includes(category))) setCategory(categories[0])
  },[categories,category])
  const filtered=useMemo(()=>{
    const q=search.toLowerCase().trim()
    return menu.filter(i=>(!category||i.category===category)&&(!q||i.name.toLowerCase().includes(q)||i.barcode===q))
  ...
  const subtotal=useMemo(()=>cartLines.reduce((sum,i)=>{
    ...
    return sum+(cart[i.id]||0)*(i.price+sizeDelta+modTotal)
  },0),[cartLines,cart,mods,selectedSizes])
  ...
  const addProduct=(item:MenuItem)=>{
    if(item.sizes?.length){setSizePicker(item);return}
    qty(item.id,1)
  }
  const chooseSize=(item:MenuItem,size:{id:number;name:string;price_delta:number})=>{
    setSelectedSizes(s=>({...s,[item.id]:size}))
    qty(item.id,1)
    setSizePicker(null)
  }
  ...
  const openShift=async()=>{
    ...
    const s:any=await api('/shifts/open',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({opening_cash:+opening||0,staff_id:loggedUser?.id||null})})
  ...
  const closeShift=async()=>{
    ...
    if(!pin)return
    try{
      const r:any=await api(`/shifts/${shift.id}/close`,{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({actual_cash:+actual||0,staff_pin:pin})
      })
      setClosingReport(r)
  ...
  const submit=async(hold=false)=>{
    const items=Object.entries(cart).filter(([,q])=>q>0).map(([id,q])=>({
      menu_item_id:+id,qty:q,size_id:selectedSizes[+id]?.id||null,modifier_ids:mods[+id]||[]
    }))
    ...
    try{
      const created:any=await api('/orders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
        ...
        card_paid:pay==='split'?cardPaid:null,table_id:null,waiter_id:loggedUser?.id||waiterId||null,
        customer_id:customerId||null,shift_id:shift?.id||null,discount,coupon_code:coupon,
        delivery_address:deliveryAddress,hold
      })})
      setLastOrderId(created.id);setCart({});setMods({});setSelectedSizes({});setDiscount(0);setCoupon('');setCashPaid(0);setCardPaid(0)
  ...
  return <div className={`pos-workspace ${cashierCompact?'cashier-clean-workspace':''} card-${settings?.cashier_card_size||'auto'}`}>
    <div className="product-area">
      ...
      <div className="product-grid">{filtered.map(item=><div className="product-card-wrap" key={item.id}><button className="product-card" onClick={()=>addProduct(item)}><div className="product-photo">{item.image?<img src={item.image} alt={item.name}/>:<span>{item.name[0]}</span>}</div><div className="product-info"><small>{item.category}</small><strong>{item.name}</strong><b>{item.sizes?.length?`From ${money(Math.min(...item.sizes.map(s=>item.price+s.price_delta)))}`:money(item.price)}</b></div>{(cart[item.id]||0)>0&&<em>{cart[item.id]}</em>}</button>{(cart[item.id]||0)>0&&item.modifiers?.length?<div className="modifier-mini">{item.modifiers.map(m=><label key={m.id}><input type="checkbox" checked={(mods[item.id]||[]).includes(m.id)} onChange={()=>toggleMod(item.id,m.id)}/>{m.name} +{m.price}</label>)}</div>:null}</div>)}</div>
    </div>
    <aside className="order-panel">
      <div className="order-panel-head"><div><span>Current order</span><strong>{type}</strong></div><button className="ghost-danger" onClick={()=>{setCart({});setMods({});setSelectedSizes({})}}>Clear</button></div>
      ...
      <div className="order-summary">
        ...
        {type==='delivery'&&<input className="plain-input" placeholder="Delivery address" value={address} onChange={e=>setAddress(e.target.value)}/>}
    ...
    {sizePicker&&<div className="size-picker-backdrop" onClick={()=>setSizePicker(null)}>
      <div className="size-picker" onClick={e=>e.stopPropagation()}>
        <div className="size-picker-head"><div><small>SELECT SIZE</small><strong>{sizePicker.name}</strong></div><button onClick={()=>setSizePicker(null)}>×</button></div>
        <div className="size-options">
          {sizePicker.sizes?.map(s=><button key={s.id} onClick={()=>chooseSize(sizePicker,s)}><span>{s.name}</span><b>{money(sizePicker.price+s.price_delta)}</b></button>)}
        </div>
      </div>
    </div>}

    {closingReport&&<div className="closing-modal-backdrop">
      <div className="closing-receipt closing-receipt-short">
        <div className="closing-receipt-head">
          <div>
            <b>{settings?.shop_name||'MAHI POS'}</b>
            <span>SHIFT CLOSING · #{closingReport.id}</span>
          </div>
          <button onClick={()=>setClosingReport(null)}>×</button>
        </div>

        <div className="closing-meta">
          <span>{new Date(closingReport.closed_at).toLocaleDateString('en-GB')}</span>
          <span>Cashier: <b>{closingReport.staff_name}</b></span>
        </div>

        <div className="closing-lines">
          <div><span>Opening Cash</span><b>{money(closingReport.starting_cash)}</b></div>
          <div><span>Cash Sales</span><b>{money(closingReport.cash_sales)}</b></div>
          <div><span>Card Sales</span><b>{money(closingReport.card_sales)}</b></div>
          <div><span>Refund</span><b>- {money(closingReport.refunds)}</b></div>
          <div><span>Cash In</span><b>+ {money(closingReport.cash_in)}</b></div>
          <div><span>Cash Out</span><b>- {money(closingReport.cash_out)}</b></div>
          <div><span>Expenses</span><b>- {money(closingReport.expenses)}</b></div>

          <hr/>

          <div className="strong"><span>NET SALES</span><b>{money(Number(closingReport.gross_sales||0)-Number(closingReport.refunds||0))}</b></div>

          <hr/>

          <div><span>Expected Cash</span><b>{money(closingReport.expected_cash)}</b></div>
          <div><span>Actual Cash</span><b>{money(closingReport.actual_cash)}</b></div>
          <div className={`strong ${Number(closingReport.difference)!==0?'difference':''}`}>
            <span>DIFFERENCE</span><b>{money(closingReport.difference)}</b>
          </div>
        </div>

        <div className="closing-footer closing-footer-short">
          <div>
            <span>Orders: <b>{closingReport.order_count}</b></span>
            <span>Closed By: <b>{closingReport.closed_by}</b></span>
          </div>
          <strong>SHIFT CLOSED</strong>
          <button className="primary-btn" onClick={()=>setClosingReport(null)}>DONE</button>
        </div>
      </div>
    </div>}
</div>
